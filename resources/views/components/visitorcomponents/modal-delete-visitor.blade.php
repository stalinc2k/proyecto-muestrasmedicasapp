...
<a onclick="mostrarModal('visitorModalDelete-{{$visitorId}}')"
...
<div id="visitorModalDelete-{{$visitorId}}" class="fixed inset-0 bg-gray-900 bg-opacity-50 items-center justify-center hidden z-50">
        ...
        <form id="formVisitor-{{$visitorId}}" action="{{route('visitor.destroy',$visitor)}}" method="POST" >
            ...
            <div class="grid gap-4 mb-4">
                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                    {{ __('Estas seguro que deseas eliminar el Representante ')}} {{$visitor->code.' - '.$visitor->name.'?'}}  <br>
                </h2>
            ...
            <div class="flex justify-end gap-2">
                <input type="hidden" name="page" value="{{ request('page', 1) }}">
                    <a onclick="cerrarModal('visitorModalDelete-{{$visitorId}}')" class="bg-orange-500 text-white px-4 py-2 rounded cursor-pointer">
                        Cancelar
                    </a>
