...
<a onclick="mostrarModal('entryModalDelete-{{$entryId}}')"
...
<div id="entryModalDelete-{{$entryId}}" class="fixed inset-0 bg-gray-900 bg-opacity-50 items-center justify-center hidden z-50">
    <div class="bg-white p-6 rounded-xl shadow-lg w-96">
        <h2 class="text-xl font-bold mb-4">Eliminar</h2>
        <form id="formentry-{{$entryId}}" action="{{route('income.destroy',$entry)}}" method="POST" >
            @csrf
            @method('DELETE')
            ...
            <div class="flex justify-end gap-2">
                <input type="hidden" name="page" value="{{ request('page', 1) }}">
                    <a onclick="cerrarModal('entryModalDelete-{{$entryId}}')" class="bg-orange-500 text-white px-4 py-2 rounded cursor-pointer">
                        Cancelar
                    </a>
